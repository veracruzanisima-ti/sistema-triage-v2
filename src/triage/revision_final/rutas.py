    ...
    *,
    ...
    lista_para_emitir = bool(productos) and not sin_preparar and all(
        ...
        or (
            ...
            and item.validacion_fiscal is not None
            and item.calculo_venta is not None
        )
        for item in productos
    )
    return request.app.state.plantillas.TemplateResponse(
        ...
        context={
            ...
            "precios_venta_confirmados": sum(
                item.precio_venta is not None
                and item.decision_comercial.estado == EstadoComercial.COTIZABLE
                for item in productos
            ),
            "no_se_cotiza": sum(
...
@router.post("/{cotizacion_id}/revision-final/{partida_id}/precio-venta")
def guardar_precio_venta(
    cotizacion_id: str,
    partida_id: str,
    request: Request,
    sesion: Sesion,
    ...
    """Confirma el precio final sin IVA sin aplicar porcentajes ni reglas automáticas."""
    ...
    try:
        ...
        mensaje = (
            "el precio unitario final sin IVA no es válido"
...
@router.post("/{cotizacion_id}/revision-final/{partida_id}/precio-venta/retirar")
def retirar_precio_venta_actual(
    cotizacion_id: str,
    partida_id: str,
    request: Request,
    sesion: Sesion,
    ...
    """Retira el precio final vigente mediante un nuevo evento auditable."""
